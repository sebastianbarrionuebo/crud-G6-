<?php
    require_once("./models/studentSubject.php");
    ...
    function handleGetS($conn) {
        ...
        if (isset($_GET['id'])) {
            ...
            $result = getAllStudentSubject($conn);  // Obtiene las materias
            $data = [];
    ...
    function handlePostS($conn) {
        ...
        if (createStudentSubject($conn, $input['student_id'], $input['subject_id'], $input['grade'])) {
    ...
    function handlePutS($conn) {
        $input = json_decode(file_get_contents("php://input"), true);
        if (updateStudentSubject($conn, $input['id'], $input['student_id'], $input['subject_id'], $input['grade'])) {
